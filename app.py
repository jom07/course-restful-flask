...
@app.route('/')
def root():
    return 'Welcome to the planets REST API'
...
def update_planet():
    ...
    if planet:
        ...
        planet.planet_type = data['planet_type']
        planet.home_star = data['home_star']
        planet.mass = float(data['mass'])
        planet.radius = float(data['radius'])
        planet.distance = float(data['distance'])
        db.session.commit()
